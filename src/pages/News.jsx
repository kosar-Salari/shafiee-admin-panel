...
export default function News() {
  ...
  const loadData = async () => {
    try {
      const categoriesResult = await window.storage.get('news-categories');
      ...
      if (newsResult) {
        setNews(JSON.parse(newsResult.value));
      }
    } catch (error) {
  ...
  const saveData = async (newCategories, newNews) => {
    try {
      await window.storage.set('news-categories', JSON.stringify(newCategories || categories));
      await window.storage.set('news-items', JSON.stringify(newNews || news));
    } catch (error) {
  ...
  const addCategory = () => {
    ...
    saveData(updatedCategories, news);
  ...
  const confirmNewsCategory = () => {
    ...
    setNews(updatedNews);
    saveData(categories, updatedNews);
    
    setShowNewsModal(false);
    window.location.href = `/builder?newsId=${newsItem.id}&category=${selectedCategory}&title=${newsForm.title}&slug=${newsForm.slug}`;
  };
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={newsForm.slug}
                      onChange={(e) => setNewsForm({ ...newsForm, slug: e.target.value.replace(/\s+/g, '-').toLowerCase() })}
