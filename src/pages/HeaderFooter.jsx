    ...
    return apiCols.map((col, idx) => ({
        ...
        links: Array.isArray(col.items)
            ? col.items.map((item, j) => ({
                id: item.id || `l-${idx}-${j}`,
                text: item.text || "",
                url: item.link || "",  // 👈 تغییر: از link بخون نه url
                icon: item.icon || "",
            }))
            : [],
    }));
    ...
    return sorted.map((col, idx) => ({
        ...
        items: Array.isArray(col.links)
            ? col.links.map((l, linkIdx) => ({
                text: l.text || "",
                link: l.url || "",  // 👈 تغییر از url به link
                icon: l.icon || "",
                position: linkIdx + 1,  // 👈 اضافه شد
            }))
            : [],
    }));
}
                        ...
                        <div className="px-4 pt-3 border-b border-gray-100 flex items-center justify-between">
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeSection === "header"
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeSection === "footer"
                                        ? "bg-indigo-50 text-indigo-700 border border-indigo-200"
                                        : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                                        }`}
                                >
